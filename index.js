...
prefix = "."
name = "~ FOX | BOT"
rdaftar = "OBRIGADO POR SE REGISTAR PARA SER UM MEMBRO DO FOX 😁"
rmenu = "Fox"
limitt = 10
ban = []
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'ESPERA AI TO FAZENDO O BAGUIO',
				success: '*Ai alek.*',
				error: {
				...
				only: {
					...
					userB: `salve Mano ${pushname2} voce nn esta cadastrado no banco de dados do FOX digite ${prefix}daftar`,
		...
		case 'daftar':
            ...
            break  
                 ...
                 case 'kalkulator':
				   ...
				   if (isLimit(sender)) return reply(limitend(pushname2))
				    ...
				    break 
				case 'owner':
                 client.sendMessage(from, {displayname: "FOXOFC", vcard: vcard}, MessageType.contact, { quoted: mek})
                 client.sendMessage(from, 'Salve o número do meu criador ae fdp:)\n wa.me/+5521979664189',text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
				if (isBanned) return reply(mess.only.benned)    
				...
				case 'ban':
					...
					if (!is) return reply(mess.only.ownerB)
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('5521979664189@s.whatsapp.net', options, text, {quoted: mek})
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Não registrado no banco de dados *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[FOXOFCBOT]','aqua'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
